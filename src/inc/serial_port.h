...
public:
	...
	*  @see:
	*/
	bool ReadChar(uint8_t *pRecved);

	//uint8_t *MotorMoveXY(unsigned char x, unsigned char y);//xy相对移动
	//uint8_t *StopMotor(unsigned char sm1);
	//uint8_t *SetSpeed(AXIS_TYPE enAxisType, int speed);
	//uint8_t *SetRunSpeed(int TY, int speed);
private:
